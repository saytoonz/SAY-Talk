...
import com.nsromapa.frenzapp.saytalk.firebase.MessagingService
import com.nsromapa.frenzapp.saytalk.models.Models
import com.nsromapa.frenzapp.saytalk.utils.DateFormatter
...
import com.nsromapa.frenzapp.saytalk.views.ColorGenerator
import com.nsromapa.frenzapp.saytalk.views.Holders
...
//            .child(utils.constants.FILE_TYPE_VIDEO).child(messageID)
    ...
    //// Check if targeted id is having an on going call or...
    //// any initiated calls.....
    private fun hasTargetInitiatedCallAlready(callId: String, audioOnly: Boolean) {
        FirebaseUtils.ref.callRef(targetUid)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    if (p0.exists()) {
                        val isUser_or_Group: String
                        if (targetType == FirebaseUtils.KEY_CONVERSATION_SINGLE) isUser_or_Group =
                            "$nameOrNumber is already on a call"
                        else isUser_or_Group = "$nameOrNumber has already initiated a meeting would you like to join?"
                        AlertDialog.Builder(context)
                            .setMessage(isUser_or_Group)
                            .setPositiveButton("Ok", null)
                            .setNegativeButton("Retry") { _, _ ->
                                hasTargetInitiatedCallAlready(callId, audioOnly)
                            }
                            .show()
                    } else {
